...
int main(){
  double a=-10.0,b=50.0,x,h,y1,y2,eps=pow(2.0,-30.0);
  int n=3;
  ...
  for(x=a+h;x<=b;x+=h){
    y2=f(x);
    if(y1*y2<0.0){
      printf("x=%f",bisection(x-h,x,eps));
...
void newton(double x){
  int n=0;double d,x0=x,x1=x,x2=x,m;
  do{
    d=-f(x)/df(x);
    x=x+d;
    n++;
  ...
  m=(x0-x1)/(x0-2.0*x1+x2);
  if(n==NMAX){
    printf("no solution\n");
  }
  else{
    printf("solution is x=%f %f\n",x,m);
