...
static void PrsInit(optmod *opt,
                    optpar *par)
{
  int     m=opt->m,n=opt->n,
          nz=opt->nz,i,j,k,nn0;
                       ...
                       optpar *par)
{
  ...
  for (j=0; j<n; j++) {
    ...
    if (!nn) {
      ...
      if (cj>0.0)      xj=opt->l[j];
      else if (cj<0.0) xj=opt->u[j];
             ...
             optmod *opt)
{
  ...
  if (par->prlev) {
	if (INFM_PRE)printf(" BEGIN presolving (level=" IFMT ")\n",par->prlev);
    ...
    do {
      ...
      if (par->prlev<=3) continue;
      
      SetColList(n,&front,&rear,opt,iw,ix);
      idproc=ChkRowDupl(m,n,rear,par,opt,iw,ix);
      if (!idproc) return false;
      
      if (par->prlev<=4) continue;
    ...
    LeftDots(ss,43);
    if(INFM_PRE)printf("   linear dependent rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->nrnul);
    LeftDots(ss,55);
    if(INFM_PRE)printf("   null rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->nrsg);
    LeftDots(ss,50);
    if(INFM_PRE)printf("   singleton rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->nrdb);
    LeftDots(ss,50);
    if(INFM_PRE)printf("   doubleton rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->nrfc);
    LeftDots(ss,52);
    if(INFM_PRE)printf("   forcing rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->nrdm);
    LeftDots(ss,50);
    if(INFM_PRE)printf("   dominated rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->nrdu);
    LeftDots(ss,50);
    if(INFM_PRE)printf("   duplicate rows "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->ncnul);
    LeftDots(ss,52);
    if(INFM_PRE)printf("   null columns "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->ncfx);
    LeftDots(ss,51);
    if(INFM_PRE)printf("   fixed columns "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->ncdm);
    LeftDots(ss,47);
    if(INFM_PRE)printf("   domimated columns "SFMT"\n",ss);
    
    sprintf(ss," %d",opt->ncdu);
    LeftDots(ss,47);
    if(INFM_PRE)printf("   duplicate columns "SFMT"\n",ss);
    
	if (INFM_PRE)printf(" END presolving\n");
  }
  ...
  if (opt->n>0&&opt->m>0) {
    sprintf(ss," %.3f%s",
                100.0*(((double)opt->nz/
                (double)opt->m)/(double)(opt->n)),"%");
    LeftDots(ss,57);
    if(INFM_FMD)printf("   density "SFMT"\n",ss);
    ...
    sprintf(ss," %.3f%s : "IFMT"/("IFMT"*"IFMT")",
			   ...
			   "%",opt->nz,opt->m,opt->n);
    LeftDots(ss,53);
    if(OUT_FILE)fprintf(fout,"   final model "SFMT"\n\n",ss);
  }
  else {
    opt->pobj=opt->obj;
    opt->dobj=opt->obj;
  ...
  LeftDots(ss,60);
  if(INFM_FMD)printf("   rows " SFMT "\n",ss);
  
  sprintf(ss," "IFMT,opt->n);
  LeftDots(ss,57);
  if(INFM_FMD)printf("   columns " SFMT "\n",ss);
  
  sprintf(ss," "IFMT,opt->nz);
  LeftDots(ss,56);
  if(INFM_FMD)printf("   nonzeros " SFMT "\n",ss);
  
  sprintf(ss," "IFMT,opt->nub);
  LeftDots(ss,52);
  if(INFM_FMD)printf("   upper bounds " SFMT "\n",ss);
  
  sprintf(ss," "IFMT,opt->nfr);
  LeftDots(ss,52);
  if(INFM_FMD)printf("   free columns " SFMT "\n",ss);
