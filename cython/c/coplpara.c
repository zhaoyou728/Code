              ...
              optmod *opt)
{
  ...
  par->mcol		= DEFAULT_PAR_MAX_COLSZ;
  par->melm		= DEFAULT_PAR_MAX_ELMSZ;
  
  par->iterlmt	=  DEFAULT_PAR_MAX_ITER;
  par->cachesz	= DEFAULT_PAR_MAX_CACHESZ;
  
  par->save     =                 false;
  par->prlev	=     DEFAULT_PAR_PRLEV;
  par->prlop	=     DEFAULT_PAR_PRLOP;
  par->cross	=     DEFAULT_PAR_CROSS;
  ...
  par->lower	=	  DEFAULT_PAR_LOWER;
  par->upper	=     DEFAULT_PAR_UPPER;
  ...
  if (fp) {
    ...
    printf("   file name                   : "
           SFMT"\n",ss);
    if(OUT_FILE)fprintf(fout,"SPC FILE\n");
    ...
    while (!feof(fp)) {
      ...
      ncard++;
      if(OUT_FILE)fprintf(fout,I6FT""SPC4,ncard);
	  if (OUT_FILE)fprintf(fout, "%s", ss);
      //fputs(ss,fout);
      if (*ss=='*'||*ss=='\n'||*ss=='\0')
        continue;
      ...
      switch(k) {
        ...
        case CROS:
          ...
          else if(OUT_FILE)fprintf(fout," this setting ignored\n");
          break;
        case DENS:
          par->idden=atoi(val);
          if (par->idden<1)
            par->idden=1;
          printf("   dense column threshold      : "
                 IFMT"\n",par->idden);
          break;
        case DINF:
          par->told=atof(val);
        ...
        case NUMC:
          ...
          if (par->mcol<0)
            par->mcol=-par->mcol;
          printf("   maximum number of columns   : "
                 IFMT"\n",par->mcol);
          break;
        case NUME:
          par->melm=atoi(val);
        ...
        case UPPE:
          ...
          break;
        default:
          if(OUT_FILE)fprintf(fout,"   no such setting\n");
          break;
      }
    }
