...
void ChlFree(chol **cl)
{
  ...
  if (*cl) {
    if (r->iaat)  iFree(&r->iaat);
    if (r->naat)  iFree(&r->naat);
    if (r->jaat)  iFree(&r->jaat);
    if (r->ud)    dFree(&r->ud);
    if (r->iju)   iFree(&r->iju);
    if (r->iu)    iFree(&r->iu);
    if (r->jun)   iFree(&r->jun);
    if (r->ju)    iFree(&r->ju);
    if (r->un)    dFree(&r->un);
    if (r->perm)  iFree(&r->perm);
    if (r->invp)  iFree(&r->invp);
    if (r->isnd)  iFree(&r->isnd);
    if (r->idn)   iFree(&r->idn);
    if (r->dmj)   iFree(&r->dmj);
    if (r->jdn)   iFree(&r->jdn);
    free(r);
  }
  *cl=NULL;
} /* ChlFree */
