...
int SetTime(clock_t tim[],int phase)
{
  int i;
...
void PrintHead(void)
{
  ...
  printf("\n\n");
  //for (i=0; i<8; i++) printf("%s", Title[i]);
  printf("\n *************************** COPL STARTS ");
  printf("***************************\n\n");
...
int FprintHead(void)
{
  if (!FilesOpen()) return false;
  if(OUT_FILE)fprintf(fout,"====================\n");
  if(OUT_FILE)fprintf(fout,"C O P L  1.0\n");
  if(OUT_FILE)fprintf(fout,"====================\n\n");
  return true;
} /* FprintHead */
...
void PrintEnd(clock_t tim[])
{
  ...
  LeftDots(ss,39);
  if(TIME_SHOW)printf(" time for initial data input "SFMT"\n",ss);
  if(OUT_FILE)fprintf(fout," time for initial data input "SFMT"\n",ss);
  ...
  sprintf(ss," "TFMT" sec",tmproc);
  LeftDots(ss,41);
  if (TIME_SHOW)printf(" time for presolve process "SFMT"\n",ss);
  if(OUT_FILE)fprintf(fout," time for presolve process "SFMT"\n",ss);

  tmproc =(double)(tim[SYBOL]-tim[PRSLV])*tmscal;
  sprintf(ss," "TFMT" sec",tmproc);
  LeftDots(ss,37);
  if (TIME_SHOW)printf(" time for symbolic computation "SFMT"\n",ss);
  if(OUT_FILE)fprintf(fout," time for symbolic computation "SFMT"\n",ss);
  
  tmproc =(double)(tim[NUMER]-tim[SYBOL])*tmscal;
  sprintf(ss," "TFMT" sec",tmproc);
  LeftDots(ss,36);
  if (TIME_SHOW)printf(" time for numerical computation "SFMT"\n",ss);
  if(OUT_FILE)fprintf(fout," time for numerical computation "SFMT"\n",ss);
  ...
  sprintf(ss," "TFMT" sec",tmproc);
  LeftDots(ss,35);
  if (TIME_SHOW)printf(" time for cross-over computation "SFMT"\n",ss);
  if(OUT_FILE)fprintf(fout," time for cross-over computation "SFMT"\n",ss);
  ...
  if(OUT_FILE)fprintf(fout," time for the whole process "SFMT"\n",ss);
