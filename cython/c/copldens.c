                       ...
                       double *diag)
{
  ...
  for (k=0; k<iter&&norm>=(*ptol); k++) {
    ...
    if (!k) {
      ...
      if (fabs(ztr0)<1.0E-15*ztrk) {
        if(OUT_FILE)fprintf(fout,"   preconditioner " "conjugate grandient"  " failed\n");
        dFree(&opt->pcgbf);
        return false;
      }
  ...
  if(OUT_FILE)fprintf(fout," PCG reduced it to %5.1e in %d iterations\n",
          norm/nrhs,k+1);
  if (k>=iter) 
    if(OUT_FILE)fprintf(fout,  "     (Max PCG iterations exceeded," " use the latest answer ...)\n");
  *ptol=norm;
                    ...
                    double *x)
{
  ...
  if (normr>pcgrf) {
    if(OUT_FILE)fprintf(fout," relative residual = " E3FT "\n",normr/normb);
    normr0=normr;
    for (i=0; i<m; i++)
      dtmp[i]=x[i];
    ...
    if (pcgrf>normr0) {
      if(OUT_FILE)fprintf(fout," no improvement achieved.\n");
      for (i=0; i<m; i++)
        x[i]=dtmp[i];
    }
  }
