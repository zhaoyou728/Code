...
static void XcrMsg(prfinfot *prfinf,
                   gsdec   *lu)
{
  gsmsg *linfo;
  char    ss[80];
  ...
  if(OUT_FILE)fprintf(fout," Cross-over Partition message\n");
  
  if(INFM_CRS)printf("\n   Partition message\n");
  sprintf(ss," " IFMT,prfinf->psze);
  LeftDots(ss,46);
  if(INFM_CRS)printf("     number of SB set " SFMT "\n",ss);
  LeftDots(ss,48);
  if(OUT_FILE)fprintf(fout,"   number of SB set " SFMT "\n",ss);
  
  sprintf(ss," " IFMT,prfinf->pbsze);
  LeftDots(ss,41);
  if(INFM_CRS)printf("     number of basis in SB " SFMT "\n",ss);
  LeftDots(ss,43);
  if(OUT_FILE)fprintf(fout,"   number of basis in SB " SFMT "\n",ss);
  
  sprintf(ss," " IFMT,prfinf->zsze);
  LeftDots(ss,46);
  if(INFM_CRS)printf("     number of NB set " SFMT "\n",ss);
  LeftDots(ss,48);
  if(OUT_FILE)fprintf(fout,"   number of NB set " SFMT "\n",ss);
  
  sprintf(ss," " IFMT,prfinf->zbsze);
  LeftDots(ss,41);
  if(INFM_CRS)printf("     number of basis in NB " SFMT "\n",ss);
  LeftDots(ss,43);
  if(OUT_FILE)fprintf(fout,"   number of basis in NB " SFMT "\n",ss);
  
  sprintf(ss," " IFMT,prfinf->asze);
  LeftDots(ss,37);
  if(INFM_CRS)printf("     number of basis in AT set " SFMT "\n",ss);
  LeftDots(ss,39);
  if(OUT_FILE)fprintf(fout,"   number of basis in AT set " SFMT "\n",ss);
  
  if(INFM_CRS)printf("   Iteration message\n");
  if(OUT_FILE)fprintf(fout," Cross-over Iteration Message\n");
  
  sprintf(ss," " IFMT,prfinf->itep);
  LeftDots(ss,35);
  if(INFM_CRS)printf("     number of primal iterations " SFMT "\n",ss);
  LeftDots(ss,37);
  if(OUT_FILE)fprintf(fout,"   number of primal iterations " SFMT "\n",ss);
  
  sprintf(ss," " IFMT,prfinf->ited);
  LeftDots(ss,37);
  if(INFM_CRS)printf("     number of dual iterations " SFMT "\n",ss);
  LeftDots(ss,39);
  if(OUT_FILE)fprintf(fout,"   number of dual iterations " SFMT "\n",ss);
  
  sprintf(ss," %d",prfinf->itec);
  LeftDots(ss,36);
  if(INFM_CRS)printf("     number of clean iterations " SFMT "\n",ss);
  LeftDots(ss,38);
  if(OUT_FILE)fprintf(fout,"   number of clean iterations " SFMT "\n",ss);
  
  linfo=&lu->info;
  
  if(INFM_CRS)printf("   Final results\n");
  if(OUT_FILE)fprintf(fout," Cross-over Optimization Results\n");
  
  sprintf(ss," %3.1e",prfinf->pfeas1+prfinf->mpbi);
  LeftDots(ss,42);
  if(INFM_CRS)printf("     primal infeasibility " SFMT "\n",ss);
  LeftDots(ss,44);
  if(OUT_FILE)fprintf(fout,"   primal infeasibility " SFMT "\n",ss);
  
  sprintf(ss," %3.1e",prfinf->dfeas1+prfinf->mdbi);
  LeftDots(ss,44);
  if(INFM_CRS)printf("     dual infeasibility " SFMT "\n",ss);
  LeftDots(ss,46);
  if(OUT_FILE)fprintf(fout,"   dual infeasibility " SFMT "\n",ss);
  
  sprintf(ss," %.9e",prfinf->pobj);
  LeftDots(ss,40);
  if(INFM_CRS)printf("     primal objective value " SFMT "\n",ss);
  LeftDots(ss,42);
  if(OUT_FILE)fprintf(fout,"   primal objective value " SFMT "\n",ss);
  
  sprintf(ss," %.9e",prfinf->dobj);
  LeftDots(ss,42);
  if(INFM_CRS)printf("     dual objective value " SFMT "\n",ss);
  LeftDots(ss,44);
  if(OUT_FILE)fprintf(fout,"   dual objective value " SFMT "\n",ss);
  ...
  if (bopt) fprintf(fres,"COPT&");
  else fprintf(fres,"CUNK&");
#endif
} /* XcrMsg */
             ...
             double     wrmem[])
{
  ...
  if(INFM_CRS)printf(" BEGIN cross-over...\n");
  ...
  if(INFM_CRS)printf(" END cross-over\n\n");
             ...
             double  xb[],  double  yb[])
{
  ...
  pri  =2+2*nrow+2*ncol
         +max(11*nrow,3*nrow+3*ncol);
  dua  =2+3*nrow+1*ncol
         +max(8*nrow+max(6*nrow,1*ncol),2*nrow+3*ncol);
