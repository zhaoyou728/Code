...
void ShowHead(void)
{
  printf("\n ITER"SPC8"POBJ"SPCE"DOBJ"
         SPCA"GAP"SPC6"PINF"SPC5"DINF\n");
  if(OUT_FILE)fprintf(fout," ITER"SPC8"POBJ"SPCE "DOBJ"SPCA"GAP"SPC6"PINF"SPC5"DINF\n");
} /* ShowHead */

void ShowInfor(optmod *ot)
{
  printf(" "I4FT"  "EGFT"  "EGFT"  "E3FT"  "
         E3FT"  "E3FT"\n",ot->iter,ot->pobj,
         ot->dobj,ot->rgap,ot->rrpnm,ot->rrdnm);
                   
  if(OUT_FILE)fprintf(fout," "I4FT"  "EGFT"  "EGFT"  "E3FT "  "E3FT"  "E3FT"\n",ot->iter, ot->pobj,ot->dobj,ot->rgap,ot->rrpnm,ot->rrdnm);
} /* ShowInfor */
                       ...
                       double   *rmem)
{
  ...
  if (opt->ista==ProcOk) {
    ...
    *rg      =opt->kap+opt->cx-opt->by;
    ...
    opt->rrdnm=opt->rdnm/(opt->tau
                         +sqrt(dSums(nlb,z,bid)
                         +dSums(nub,zu,NULL)
                         +dSums(m,y,NULL)
                         +dSums(nub,yu,NULL)));
                         ...
                         int    rsze, double *rmem)
{
  ...
  *dkap=((*dkap)-opt->kap*rtmp)/tau;
  ...
  rtmp=opt->rho/opt->tau;
  for(k=0; k<nfr; ++k)
    fl[bf[k]]-=(opt->c[bf[k]]-rtmp*opt->xf[k]*x[bf[k]])*(*dtau)
               +opt->rho*opt->xf[k]*dx[bf[k]];
                         ...
                         double *rmem)
{
  ...
  opt->mu0 =(dDots(nlb,x,z,bid)+dDot(nub,xu,zu)
	        +opt->tau*opt->kap)/opt->nall;
  opt->tau0=opt->tau;
  opt->kap0=opt->kap;
  mu       =opt->mu0;
  ...
  for (h=0; h<end; h++) {
    ...
    afine=afine&&(rpnorm+rdnorm+rgnorm<=opt->smdt/10.0
               || sqrt(rpnorm+rdnorm+rgnorm)<=1.0e-8);
    ...
    if (afine) {
      ...
      if(OUT_FILE)fprintf(fout," inaccurate affine search direction.\n");
      phi      =0.0;
    }
    ...
    if (!h) {
      ...
      for(k=0; k<nlb; ++k)
        dz[bid[k]]=-x[bid[k]]*z[bid[k]]+alfp
                   -phi*(dx[bid[k]]*dz[bid[k]]-rtmp);
      for(k=0; k<nfr; ++k)
        dz[bidf[k]]=0.0;
      for(k=0; k<nub; ++k)
  ...
  Tm = GetTime();
    ...
    for(idir=0; idir<4&&opt->ista==ProcOk&&rbac&&!rfac; ++idir) {
      ...
      if (phi!=0.0) {
        ...
        if (pid&&(rpnorm+rdnorm+rgnorm<=opt->smdt/10.0
               ||sqrt(rpnorm+rdnorm+rgnorm)<=1.0e-8)) {   
          if (idir==0) {
          ...
          if(OUT_FILE)fprintf(fout," inaccurate search direction.\n");
          opt->ista=ProcStall;
          r        =0.1;
          phi      =0.0;
        }
      ...
      for(k=0; k<nlb; ++k)
        dz[bid[k]]=-x[bid[k]]*z[bid[k]]+gamma*mu
                   -phi*(dx[bid[k]]*dz[bid[k]]-rtmp);
      for(k=0; k<nfr; ++k)
        dz[bidf[k]]=0.0;
      for(k=0; k<nub; ++k)
             ...
             optpar  *par)
{
  ...
  LeftDots(ss,51);
  if(INFM_FMD)printf("   dense columns " SFMT "\n\n",ss);
