               ...
               int    *ix)
{
  ...
  for (i=0; i<m; i++) {
    ...
    if (ix[i]) li=-MaxPositive;
    else li=opt->rlb[i];
    
    if (iw[i]) ui=MaxPositive;
    else ui=opt->rub[i];
    ...
    for (k=0; k<ai->nn0; k++) {
      ...
      if ((aij>0 && (ui-xx)<par->aijtol)
        ||(aij<0 && (xx-li)<par->aijtol)) {
        lj=opt->u[j];
        Char1('u');
      }
      else Char1('l');
               ...
               int    *ix)
{
  ...
  for (j=front; j>=0; j=ix[j]) {
    ...
    if (aij*aik>0) {
      if (lj<-par->bplus) uk=MaxPositive;
      else uk=(bi-aij*lj)/aik;
      if (uj>par->bplus) lk=-MaxPositive;
      else lk=(bi-aij*uj)/aik;
    }
    
    else {
      if (lj<-par->bplus) lk=-MaxPositive;
      else lk=(bi-aij*lj)/aik;
      if (uj>par->bplus) uk=MaxPositive;
      else uk=(bi-aij*uj)/aik;
    }
               ...
               int    *ix)
{
  ...
  for (j=0; j<n; j++) {
    ...
    for (k=0; k<aj->nn0; k++) {
      ...
      if (aij>0) {
        if (li<-par->bplus) ilow++;
        else ej+=aij*li;
        if (ui>par->bplus) iupp++;
        else dj+=aij*ui;
      }
      
      else {
        if (li<-par->bplus) iupp++;
        else dj+=aij*li;
        if (ui>par->bplus) ilow++;
        else ej+=aij*ui;
      }
    }
    
    if (ilow) cej=MaxPositive;
    else cej=cj-ej;
    if (iupp) cdj=-MaxPositive;
    else cdj=cj-dj;
    ...
    if (cej>0&&cdj<0) {
      ...
      if (opt->u[j]>par->bplus&&ilow<2) {
        ...
        for (k=0; k<aj->nn0; k++) {
          ...
          if (aij>0) {
            ui=opt->rlb[i];
            if (ui<-par->bplus) ui=(cj-ej)/aij;
            else if (ilow) ui=MaxPositive;
            else ui+=cej/aij;
            ...
            li=opt->rub[i];
            if (li>par->bplus) li=(cj-ej)/aij;
            else if (ilow) li=-MaxPositive;
            else li+=cej/aij;
          ...
          if (ppp) {
            ...
            if (z<2) {
              ...
              if (iii) {
                ...
                if (aij*akl>0) {
                  if (uj>par->bplus) lg=-MaxPositive;
                  else lg=(bk-aij*uj)/akl;
                  if (lj<-par->bplus) ug=MaxPositive;
                  else ug=(bk-aij*lj)/akl;
                }
                
                else {
                  if (lj<-par->bplus) lg=-MaxPositive;
                  else lg=(bk-aij*lj)/akl;
                  if (uj>par->bplus) ug=MaxPositive;
                  else ug=(bk-aij*uj)/akl;
                }
