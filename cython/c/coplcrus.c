              ...
              optmod *opt)
{
  ...
  if(INFM_CRU)printf(" BEGIN crushing...\n");
  ...
  for (j=0; j<n; j++) {
    ...
    else if (fabs(lj)<par->aijtol) continue;
    
    else if (lj>=-par->bplus) 
      TreatBound(lj,j,par,opt,LOW);
    
    else if (uj<=par->bplus) //-oo ~ uj
  ...
  if (INFM_CRU)printf(" END crushing\n");
  if (INFM_CRU)printf(" DENSITY\n");
  sprintf(ss," %.3f%s : "IFMT"/("IFMT"*"IFMT")",
         100.0*(((double)opt->nz/
         (double)opt->m)/(double)opt->n),
		 "%",opt->nz,opt->m,opt->n);
  LeftDots(ss,50);
  if (INFM_CRU) printf("   after crushing "SFMT"\n\n", ss);
  if(OUT_FILE)fprintf(fout,"   after crushing "SFMT"\n",ss);
