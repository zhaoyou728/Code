             ...
             solpt *pt)
{
  ...
  switch (key) {
    ...
    case CDUP:
      ...
      if (rtmp>0.0) {
        if (uj<= 1e29) ymin=(cj-uj)/rtmp;
        else ymin=-1e30;
        if (lk>=-1e29) ymax=(cj-lj)/rtmp;
        else ymax=1e30;
        ymin=max(ymin,lk);
        ymax=min(ymax,uk);
      }
      else {
        if (lj>=-1e29) ymin=(cj-lj)/rtmp;
        else ymin=-1e30;
        if (uk<= 1e29) ymax=(cj-uj)/rtmp;
        else ymax= 1e30;
        ymin=max(ymin,lk);
        ymax=min(ymax,uk);
      }
    ...
    case NMES:
	  ...
	  pt->r = dAlloc(pt->m0, "no");
	  pt->b = dAlloc(pt->m0, "no");
	  pt->rp = cAlloc(pt->m0, "no");
	  pt->l = dAlloc(pt->n0, "no");
	  pt->u = dAlloc(pt->n0, "no");
    ...
    case RFRC:
      ...
      if (!pt->subi)
		  pt->subi = iAlloc(pt->n, "no");
      if (!pt->ai)
		  pt->ai = dAlloc(pt->n, "no");
      ...
      while (ckey!='e') {
        ...
        rtmp=cj/aij;
        if (aij>0.0) {
          if (ckey=='l') {if (ymin>rtmp) ymin=rtmp;}
          else           {if (ymax<rtmp) ymax=rtmp;}
        }  
        else {
          if (ckey=='l') {if (ymax<rtmp) ymax=rtmp;}
          else           {if (ymin>rtmp) ymin=rtmp;} 
        }
