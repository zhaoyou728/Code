...
def mycallback(model, where):
    if where == GRB.Callback.POLLING:
        ...
        if abs(objbst - objbnd) < 0.9 * (1.0 + abs(objbst)):
            model.params.MIPFocus=1
            print('Stop early - 10% gap achieved')
            #model.terminate()
        if nodecnt >= 8268 and solcnt:
            model.params.MIPFocus=1
...
setParam("Heuristics", 0.5)
setParam("*Interval", 10)
#setParam("BranchDir", 1)
#setParam("ZeroObjNodes",1000)
#setParam("Method",1)
setParam("ImproveStartNodes",10)
setParam("RINS",100)
